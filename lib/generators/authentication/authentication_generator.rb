class AuthenticationGenerator < Rails::Generators::Base
  ...
  def add_oauth_config_to_application_yml
    ...
    oauth_config = <<~YAML
      ...
      # CLACKY_AUTH_CLIENT_ID: ''
      # CLACKY_AUTH_CLIENT_SECRET: ''
      # CLACKY_AUTH_DOMAIN: ''
...
#{match}
  ...
  def add_authentication_to_actioncable_connection
    ...
    # Add authentication methods to existing connection
    connection_content = File.read(connection_file)

    # Check if it already has identified_by :current_user
    unless connection_content.include?('identified_by :current_user')
      # Add identified_by after the class declaration
      gsub_file connection_file, /(class Connection < ActionCable::Connection::Base)\s*\n/ do |match|
        "#{match.strip}\n    identified_by :current_user\n\n"
      end
    end

    # Check if it already has a connect method
    connection_content = File.read(connection_file) # Re-read after potential identified_by insertion
    if connection_content.include?('def connect')
      say "Connect method already exists, please manually add authentication logic", :yellow
    else
